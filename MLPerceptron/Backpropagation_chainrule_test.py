...
def apple_graph() :
    ...
    d_apple_total, d_ax = layer_tax.backward(d_total)
...
def fruit_grapth() :
   ...
   # 사과와 망고의 계산 합 층
    ...
    d_apple_price, d_apple_count = layer_fruit.backward(d_apple_total)
    d_mango_price, d_mango_count = layer_fruit.backward(d_mango_total)
...
def back_propagation() :
    ...
    for i in range(100) :
        ...
        # 순전파
        apple_total = layer_apple.foraward(apple_price, apple_count) # 사과 총액
        mango_total = layer_mango.foraward(mango_price, mango_count) # 망고 총액
        fruit_total = layer_fruit.foraward(apple_total, mango_total) # 과일 총액
        total = layer_tax.foraward(fruit_total, tax) # 과일 총액 * 세금

        print('foward :', total)

        # 역전파
#        d_total = 1.0
        d_total = total - target
