...
mlp = MLPClassifier(solver='lbfgs', random_state = 0, hidden_layer_sizes = [4,2,3]).fit(X, y) # 다중 레이블 분류 지원, 히든 계층과 히든 노드 개수를 지정 후 학습
...
# MLP의 계층별 가중치 확인
print('len(mlp.coefs_) :', len(mlp.coefs_) , "mlp.coefs_ 값", mlp.coefs_)
